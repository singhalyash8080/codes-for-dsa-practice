...
node *ll_merge(node *head1, node *head2){
    ...
    if(head1->data < head2->data){
        c=head1;
        c->next = ll_merge(head1->next,head2);
    }
    else{
        c=head2;
        c->next = ll_merge(head1,head2->next);
    }
...
void solve()
{
    ...
    ll_insertAtHead(head2,6);
    ll_insertAtHead(head2,5);
    ll_insertAtHead(head2,4);
    ...
    node *mergedLL = ll_merge(head,head2);
    ll_print(mergedLL);
